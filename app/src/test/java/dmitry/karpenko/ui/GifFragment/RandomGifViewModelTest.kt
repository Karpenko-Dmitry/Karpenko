...
class RandomGifViewModelTest {
    ...
    private val gifs : Array<Gif> = arrayOf(
        Gif(0,Category.RANDOM.value,"description1","url"),
        Gif(0,Category.RANDOM.value,"description2","url")
    )

    @Before
    fun setupViewModel() {
        repository = FakeRepository(gifs,2)
        gifViewModel = RandomGifViewModel(repository)
        gifViewModel.findCurrentPos()
    }

    @Test
    fun category_RandomCategory() {
        assertThat(gifViewModel.category, `is`( Category.RANDOM))
    }
