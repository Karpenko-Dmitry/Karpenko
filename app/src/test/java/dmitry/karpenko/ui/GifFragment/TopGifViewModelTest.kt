...
class TopGifViewModelTest {
    private lateinit var gifViewModel: GifViewModel
    private lateinit var repository: FakeRepository
    private val gifs : Array<Gif> = arrayOf(
        Gif(0,Category.TOP.value,"description1","url"),
        Gif(0,Category.TOP.value,"description2","url")
    )

    @Before
    fun setupViewModel() {
        repository = FakeRepository(gifs,2)
        gifViewModel = TopGifViewModel(repository)
        gifViewModel.findCurrentPos()
    }

    @Test
    fun category_TopCategory() {
        assertThat(gifViewModel.category, `is`( Category.TOP))
    }
