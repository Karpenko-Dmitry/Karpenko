...
class LatestGifViewModelTest {
    private lateinit var gifViewModel: GifViewModel
    private lateinit var repository: FakeRepository
    private val gifs : Array<Gif> = arrayOf(
        Gif(0,Category.LATEST.value,"description1","url"),
        Gif(0,Category.LATEST.value,"description2","url")
    )

    @Before
    fun setupViewModel() {
        repository = FakeRepository(gifs,2)
        gifViewModel = LatestGifViewModel(repository)
        gifViewModel.findCurrentPos()
    }

    @Test
    fun category_LATESTCategory() {
        assertThat(gifViewModel.category, `is`( Category.LATEST))
    }
